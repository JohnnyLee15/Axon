...
class SelectionHandlerTests(unittest.IsolatedAsyncioTestCase):
    async def test_model_selection_cancellation_preserves_current_model(self) -> None:
        manager = SessionManager.__new__(SessionManager)
        manager._ui = Mock()
        manager._ui.select_item = AsyncMock(return_value=None)
        manager._llm = Mock()
        manager._settings = Mock()
        manager._llm.get_chat_model.return_value = "model-a"
        await manager._select_model()

        manager._llm.set_chat_model.assert_not_called()
        manager._settings.set.assert_not_called()
        manager._ui.info.assert_not_called()

    async def test_model_selection_updates_model_and_persists_setting(self) -> None:
        manager = SessionManager.__new__(SessionManager)
        manager._ui = Mock()
        manager._ui.select_item = AsyncMock(return_value="model-b")
        manager._llm = Mock()
        manager._settings = Mock()
        manager._llm.get_chat_model.return_value = "model-a"
        await manager._select_model()
    ...
    async def test_chat_limit_selection_cancellation_preserves_current_limit(self) -> None:
        handlers = ChatHandlers.__new__(ChatHandlers)
        handlers._ui = Mock()
        handlers._ui.select_item = AsyncMock(return_value=None)
        handlers._llm = Mock()
        handlers._settings = Mock()
        handlers._llm.get_chat_limit.return_value = 100_000
        await handlers.set_limit()
    ...
    async def test_chat_limit_selection_updates_limit_and_persists_setting(self) -> None:
        handlers = ChatHandlers.__new__(ChatHandlers)
        handlers._ui = Mock()
        handlers._ui.select_item = AsyncMock(return_value=200_000)
        handlers._llm = Mock()
        handlers._settings = Mock()
        handlers._llm.get_chat_limit.return_value = 100_000
        await handlers.set_limit()
